...
def fit_transition_matrix(X_hist, p):
    m, N = X_hist.shape
    if N < 2:
...
def run_mssa_for_L(L, df_train, df_test, fitur_cols, df_norm_all, energy_thr):
    ...
    for r in range(1, r_star + 1):
        ...
        for p in range(1, 8):
            ...
            pred = multistep_forecast(X_hist, A_p, p, h)
...
def run_mssa_pipeline(DATA_PATH, NORM_POL_PATH, NORM_MET_PATH, OUT_DIR,
    ...
    pred_all = multistep_forecast(X_hist, A_best, bestp, h)
...
def predict_next_7_days(best_model, df_norm, fitur_cols, save_dir, last_date):
    ...
    h = 7
    pred_norm = multistep_forecast(X_hist, A_best, bestp, h)
    ...
    tanggal_prediksi = [
        last_date + timezone.timedelta(days=i)
    ...
    for col in fitur_cols:
        ...
        save_plot = os.path.join(plot_dir, f"{col}_7hari.png")
        plt.savefig(save_plot, dpi=150)
        plt.close()

    print(f"Grafik prediksi 7 hari disimpan di folder: {plot_dir}")
